...
### 線 1:AB2 三臂線上測試(P5)
- **狀態**:⛔ **棄用(2026-08-12 使用者裁定:AB2/AB3 直接棄用,不跑不判讀)**
- 原內容:ctrl(policy_4500)/ T40_last / T40_best 各 300 局;7/18 時進度 C 163+/L 150/B 150。
- 後續:若本機還有 resume_ab2 迴圈在跑,接手的 session 先殺掉 process 再確認本欄。
...
- **完訓後**:①改名 ppo_m3p1_400.pt ②同 8,555 局面跑正典 last-chance 統計出表
  ③~~啟動 AB3~~ **AB3 棄用(2026-08-12 使用者裁定,同 AB2)**
...
| **Big2MDP 忠實復刻(`planner/big2mdp/`)** | 使用者裁定(2026-08-12):完整復刻 ToG 2025 整隻 agent,**不得**使用 policy/belief/value/AW-BC 任何舊資產;branch `claude/big2mdp-lite` | 本機跑 500K 自打填表 → 離線評估 → 事前註冊後上線;背景見 `docs/paper/BIG2MDP_METHOD_NOTES.md` |
