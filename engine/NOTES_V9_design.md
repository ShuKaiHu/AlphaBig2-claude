...
## V8 乾淨 baseline (2026-06-12, 36局) — V9a 對照判決 + 真實水位修正

| 指標 | V9a 乾淨(64局) | V8 乾淨(36局) |
|------|----------------|----------------|
| 平均 | **-4.45 ± 1.72** | -9.58 ± 5.48(剔除-176場後 -4.83)|
| 截尾均值 | **-5.53** | -7.41 |
| 中位 | **-4** | -6 |
| 災難場(≤-20) | **8%**,最差 -40 | 14%,最差 **-176** |
| 線上跟牌浪費 | **15%** | (bug時代 40%)|

差 +5.13 ≈ 0.89σ(不顯著,被 V8 的 -176 尾巴主導)。**V9a 在所有穩健指標上 ≥ V8,
尾部風險大幅較佳**(跟牌紀律 → 不再有 -176 式災難)。

**重大認知修正:修正 executor 後,V8 與 V9a 對真人都輸 ~4-5 分/局。**
bug 時代的「V8 +1.80 贏真人」從來不是真的(污染狀態下的量測)。誠實的現況:
正確 executor + 目前最強模型 ≈ 每局 -4~-5。與真人的真實差距比先前以為的大。

**結論**:V9a ≥ V8(點估計、中位、截尾、尾部全優,跟牌紀律壓倒性)→ 建議扶正 V9a。
下一個槓桿 = 分級信心世界採樣器(void v2 + 拒絕採樣 + 錨定,直攻 V9a 主嫌「世界太假」),
之後 V9b 像人對手 / V9c 真人棋譜 —— 縮小與真人差距需要整條 roadmap,非單點調參。

---

## 世界採樣器 v2 實作完成 (2026-06-12, wrapper a4ac7d2 後續 commit)

設計全數落地:分級信心(實測校準表)→ per-world Bernoulli 啟用 → 單張 cap 發牌過濾
→ 對子/五張 generate-and-test 拒絕(K=12, 違反最少備胎)→ N 世界 → 全資訊 value。
看板 belief 與 MCTS 共用同一採樣器(顯示=決策心中世界)。預設 ON,BIG2_VOID=0 關閉(A/B用)。

**實作中學到的**:手寫牌型比較必錯 —— 交叉驗證抓到本規則集「四條/同花順可壓任何牌型」
(4444 壓對8、♣A2345 同花順壓對8),故 _hand_can_beat 完全委託引擎 returnAvailableActions
(本來就是 MCTS 熱路徑,夠快:120 次採樣 0.08s)。
驗證:p=1.0 時自然違反率 70% → 0/200;p=0 完全忽略;對 ♣3 的 pass 自動 p=0.20(校準的意義)。

**下一步:線上 A/B** — V9a+sampler v2(預設)vs 必要時 BIG2_VOID=0。對照 V9a 首批 -4.45。

---

## 世界採樣器 v2 線上結果 (2026-06-14, 69局) — 最強訊號,首次摸到打平

| | V9a+v2 | V9a(無v2) | V8乾淨 |
|---|--------|-----------|--------|
| 平均 | **-1.13 ± 1.98** | -4.45 ± 1.72 | -9.58 |
| 95%CI | **[-5.0,+2.7] 含0** | [-7.8,-1.1] | — |
| 出完率 | **16%** | 8% | 6% |
| 跟牌浪費 | 15% | 15% | (40%) |
| 追蹤 | 0/52, 對賬78/78 | | |

- v2 在真實局全力運作:平均 8.1 約束/決策、組合拒絕 87% 乾淨滿足。
- 比無 v2 的 V9a **+3.32 ≈ 1.27σ**(69局,未顯著但方向對、最佳)。**首次 CI 跨 0**
  (vs 打平 t=-0.57σ → 接近不輸不贏)。
- **出完率翻倍(8%→16%)是關鍵**:V9a 輸在「太被動」(全資訊 value 在虛假世界覺得留牌總是好);
  世界做真後模型變主動、搶得到控場 → **直接驗證「世界太假」是 V9a 頭號嫌疑**。

**結論:整個修正後時代最強訊號。v2 已是預設部署(checkpoint 帶 value_state 即自動啟用)。**
1.27σ 未到顯著,續累積即可(v2 預設,使用者照常玩就累積)。下一槓桿:V9b 像人對手 /
V9c 真人棋譜,把「-1 打平」推成「正分贏真人」。

---

## V9b (combo 特徵) 結果 (2026-06-15) — 失敗:特徵有了,但模型沒學會用

診斷確認 value 對牌組結構盲目(保留 vs 破壞順子 ≈ 擲銅板),於是加 8 維牌組特徵
(對/三條/四條/同花/順窗/葫蘆/最長連) 進 policy+全資訊 value。結果三項指標一致無效:

| | V9b (combo) | V9a (無) |
|---|---|---|
| value 盲目診斷(保留>破壞順子)| 47% | 44% |
| 行為拆組合率(MCTS 實打)| 72% / 可避免 43% | 66% / 41% |
| MCTS-80 vs heuristic | +2.86±1.27 | +5.64±1.50 |

**結論:加特徵(給「能力」)≠ 學會用(需「訓練訊號」)。** 這是本專案第三次同型教訓
(V7 value-coef、belief、現在 combo):**對弱 heuristic self-play,拆組合不會被懲罰 →
即使 value 看得到「我有順子」這個特徵,訓練資料裡保留組合並沒有換到更好結局 → 網路正確地
學成「組合不重要」、忽略該特徵。** 特徵是必要非充分。

→ 真正的解鎖是 **V9c:更強/像人的對手**(會懲罰拆組合、亂送大牌)。combo 特徵保留在架構裡
(預設關、自動偵測),等有了會懲罰的對手再一起發力。**V9b 不部署;V9a+v2(-1.13)維持冠軍。**
(注意:MCTS-vs-heuristic 是弱代理、不預測線上;但「targeted 診斷顯示沒達成設計目標」這點
